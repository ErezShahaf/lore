...
3. DISAMBIGUATE — `ask_clarification`: Answering now would likely mislead: “the X” with no preview match; several equally plausible “the one”; same label across plausible rows; Stripe/webhook JSON without event type when previews show multiple distinct payloads; material disagreement; ambiguous reference.
4. PLAN: Emit JSON.
</logic_flow>
