...
<logic_flow>
1. STRUCTURED BLOB: Message is only JSON/array, payment-style event payload, or similar—and no explicit storage verb (save, remember, store, keep, add to library, log) → `speak` (ask what to do); NEVER assume `save`.
