...
`forks/kind/<kind>/entry.md` adds extra guidance for that **`facts.kind`**. Outcomes in-repo: `thought_saved_single`, `thought_saved_many`, `instruction_stored`, `command_no_documents`, `command_no_match`, `command_executed`, `multi_action_summary`, and `default`.
